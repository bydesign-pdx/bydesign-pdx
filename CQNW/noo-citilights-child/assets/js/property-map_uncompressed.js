	...
	function search_initialize(){
		...
		if(mapSearchBox.length && mapBox.length){
					 ...
					 <div class="info-img">' + maker.image + '</div> \
						 <div class="info-summary"> \
						 	...
						 	<div class="info-detail"> \
					 			<div class="size"><span>' + maker.area + '</span></div> \
					 			<div class="bedrooms"><span>' + maker.bedrooms + '</span></div> \
					 			<div class="bathrooms"><span>' + maker.bathrooms + '</span></div> \
					 		</div> \
					 		<div class="info-more"> \
			...
			//create markers
			 ...
			 if($(mapBox).data('source') =='IDX' && (typeof(dsidx) == 'object') && dsidx.dataSets){
				 var idxCode = null;
				 var bounds = new google.maps.LatLngBounds();
				 ...
				 for (var i = 0; i < dsidx.dataSets[idxCode].length; i++) {
					...
					if (marker.ShortDescription !== undefined){
					 ...
					 var gmarker = new google.maps.Marker({
					    ...
					    title: title,
					    bedrooms: marker.BedsShortString,
					    bathrooms: marker.BathsShortString,
					...
					google.maps.event.addListener(gmarker, 'click', function(e) {
						var infoContent = '<div class="gmap-infobox"><a class="info-close" onclick="return infoBox.close();" href="javascript:void(0)">x</a>\
							 <div class="info-img"><img src="' + this.image + '1-full.jpg" style="width:190px;height:160px"/></div> \
								 <div class="info-summary"> \
								 	<h5 class="info-title">' + this.title + '</h5> \
								 	<div class="info-detail"> \
							 			<div class="size"><span>' + this.area + '</span></div> \
							 			<div class="bedrooms"><span>' + parseInt(this.bedrooms) + '</span></div> \
							 			<div class="bathrooms"><span>' + parseInt(this.bathrooms) + '</span></div> \
							 		</div> \
							 		<div class="info-more"> \
	...
	function search_filter(){
		...
		$('.noo_advanced_search_property').each(function(){
			...
			if($this.find('#gmap').length){
				var location 		= $this.find('input.glocation_input').length > 0 ? $this.find('input.glocation_input').val() : '',
				sub_location 	= $this.find('input.gsub_location_input').length > 0 ? $this.find('input.gsub_location_input').val() : '',
				status 			= $this.find('input.gstatus_input').length > 0 ? $this.find('input.gstatus_input').val() : '',
				category 		= $this.find('input.gcategory_input').length > 0 ? $this.find("input.gcategory_input").val() : '',
				bedrooms 		= $this.find('input.gbedroom_input').length > 0 ? $this.find('input.gbedroom_input').val() : '',
				bathrooms 		= $this.find('input.gbathroom_input').length > 0 ? $this.find('input.gbathroom_input').val() : '',
				min_price 		= $this.find('input.gprice_min').length > 0 ? $this.find('input.gprice_min').val() : 0,
				max_price 		= $this.find('input.gprice_max').length > 0 ? $this.find('input.gprice_max').val() : 0,
				min_area 		= $this.find('input.garea_min').length > 0 ? $this.find('input.garea_min').val() : 0,
				max_area 		= $this.find('input.garea_max').length > 0 ? $this.find('input.garea_max').val() : 0;
