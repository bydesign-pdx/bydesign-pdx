...
// Amenities & Features
$custom_features = NooProperty::get_feature_option('features');
$features = array();
...
$action = 'add';
$submit_title = __('Submit Property', NOO_TEXT_DOMAIN);
$submit_text = __('Add Property', NOO_TEXT_DOMAIN);
...
if( isset( $_GET['prop_edit'] ) && is_numeric( $_GET['prop_edit'] ) ) {
	...
	$action = 'edit';
	$submit_title = __('Edit Property', NOO_TEXT_DOMAIN);
	$submit_text = __('Update Property', NOO_TEXT_DOMAIN);
}
...
if( $membership_type == 'membership' ) {
	if( $membership_info['data']['featured_remain'] == 0 ) {
		$featured_permision['message'] = __('Please upgrade your membership before you can make this listing featured.', NOO_TEXT_DOMAIN);
	} else {
		if( $post_status == 'publish' ) {
			$featured_permision['allow'] = true;
			$featured_permision['message'] = __('Make this listing featured. The number of featured items will be subtracted from your package.', NOO_TEXT_DOMAIN);
		} else {
			$featured_permision['message'] = __('You can make this listing featured after your property is approved.', NOO_TEXT_DOMAIN);
		}
	}
} elseif( $membership_type == 'submission' ) {
	$featured_permision['message'] = __('Make this listing featured from Your Properties list.', NOO_TEXT_DOMAIN);
}
...
if( 'POST' == $_SERVER['REQUEST_METHOD'] ) {
	...
	if ( !isset($_POST['_noo_property_nonce']) || !wp_verify_nonce($_POST['_noo_property_nonce'],'submit_property') ){
		exit(__('Sorry, your session is expired or you submitted an invalid property form.', NOO_TEXT_DOMAIN));
	}
	...
	if( empty( $agent_id ) && empty( $submit_agent_id ) ) {
		...
		if( !$agent_id ) {
			$has_err = true;
			$err_message[] = __('There\'s an unknown error when creating an agent profile for your account. Please resubmit your property or contact Administrator.', NOO_TEXT_DOMAIN);
		}
	} elseif( $agent_id != $submit_agent_id ) {
		$has_err = true;
		$err_message[] = __('There\'s an unknown error. Please resubmit your property or contact Administrator.', NOO_TEXT_DOMAIN);
	}
	...
	if( !$has_err ) {
		...
		if( empty($title) ) {
			$has_err = true;
			$err_message[] = __('Please submit a title for your property', NOO_TEXT_DOMAIN);
		}

		if( empty($desc) ) {
			$has_err = true;
			$err_message[] = __('Please input a description for your property', NOO_TEXT_DOMAIN);
		}

		if( empty($gallery) ) {
			$has_err = true;
			$err_message[] = __('Your property needs at least one image', NOO_TEXT_DOMAIN);
		}

		if( empty($address) ) {
			$has_err = true;
			$err_message[] = __('Your property needs a specific address', NOO_TEXT_DOMAIN);
		}
	}
	...
	if( ! $has_err ) {
		...
		if( $_POST['action'] == 'add' ) {
			...
			if( !$post_id ) {
				$has_err = true;
				$err_message[] = __('There\'s an unknown error when inserting your property to database. Please resubmit your property or contact Administrator.', NOO_TEXT_DOMAIN);
			} else {
				$success = true;
				...
				if( $need_approve ) {
					$message .= sprintf( __("A user has just submitted a listing on %s and it's now waiting for your approval. To approve or reject it, please follow this link: %s", NOO_TEXT_DOMAIN), $site_name, $property_admin_link) . "\r\n\r\n";
					wp_mail($admin_email,
						sprintf(__('[%s] New submission needs approval',NOO_TEXT_DOMAIN), $site_name),
						$message,
						$headers);
				} else {
					$message .= sprintf( __("A user has just submitted a listing on %s. You can check it at %s", NOO_TEXT_DOMAIN), $site_name, $property_admin_link) . "\r\n\r\n";
					wp_mail($admin_email,
						sprintf(__('[%s] New property submission',NOO_TEXT_DOMAIN), $site_name),
						$message,
						$headers);
				}
			}
			...
			if( !empty( $post_id ) ) {
				...
				if( 0 === wp_update_post( $post ) ) {
					$has_err = true;
					$err_message[] = __('There\'s an unknown error when updating your property. Please resubmit your property or contact Administrator.', NOO_TEXT_DOMAIN);
				} else {
					$success = true;
					...
					if( $need_approve ) {
						$message .= sprintf( __("A user has just edited one of his listings and it's now waiting for your approval. To approve or reject it, please follow this link: %s", NOO_TEXT_DOMAIN), $property_admin_link) . "\r\n\r\n";
						wp_mail($admin_email,
							sprintf(__('[%s] New submission needs approval',NOO_TEXT_DOMAIN), $site_name),
							$message,
							$headers);
					} else {
						$message .= sprintf( __("A user has just edited one of his listings. You can check it at %s", NOO_TEXT_DOMAIN), $property_admin_link) . "\r\n\r\n";
						wp_mail($admin_email,
							sprintf(__('[%s] A listing has been edited',NOO_TEXT_DOMAIN), $site_name),
							$message,
							$headers);
					}
				}
	...
	<div class="main-content container-boxed max offset">
		<div class="row">
			...
			<div class="<?php noo_main_class(); ?>" role="main">   
				...
				<?php if( !isset( $_GET['listing_edit'] ) && !NooAgent::can_add( $agent_id ) ) : ?>
				<div class="submit-content">
				<h4><?php if( NooMembership::is_membership() ) {
					_e('Your current package doesn\'t let you publish properties anymore! You will have to upgrade your membership first.', NOO_TEXT_DOMAIN);
					do_shortcode( '[noo_membership_packages style="ascending" featured_item="2" ]' );
				} else {
					_e('Sorry, you don\'t have the permission to submit any property!', NOO_TEXT_DOMAIN);
				}
				?></h4>
				</div>
				<?php else : ?>
				<div class="submit-content">
					<?php if( $has_err && !empty($err_message) ) : ?>
						<div class="submit-error">
							...
							<div class="noo-message alert alert-danger alert-dimissible" role="alert">
								<button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only"><?php _e( 'Close', NOO_TEXT_DOMAIN ); ?></span></button>
								<?php echo $message; ?>
							</div>
							<?php endforeach; ?>
						</div>
					...
					<form id="new_post" name="new_post" method="post" enctype="multipart/form-data" class="noo-form property-form" role="form">
							...
							<div class="group-container row">
								...
								<div class="col-md-8">
									<div class="form-group s-prop-price row">
										<div class="price col-md-4">
											<label for="price"><?php _e('Price',NOO_TEXT_DOMAIN); ?>&nbsp;*&nbsp;(<?php echo NooProperty::get_currency_symbol(NooProperty::get_general_option('currency')); ?>)</label>
											<input type="text" id="price" class="form-control" value="<?php echo $price; ?>" name="price" required />
										</div>
										<div class="price_label col-md-8">
											<label for="price_label"><?php _e('After price label (ex: "per month")',NOO_TEXT_DOMAIN); ?></label>
								...
								<div class="col-md-4">
									<div class="form-group s-prop-status">
										<label><?php _e('Status',NOO_TEXT_DOMAIN); ?></label>
										<div class="dropdown">
							   				<?php 
								...
								<div class="col-md-4">
									<div class="form-group s-prop-type">
										<label><?php _e('Type',NOO_TEXT_DOMAIN); ?></label>
										<div class="dropdown">
							   				<?php 
								...
								<div class="col-md-4">
									<div class="form-group s-prop-bedrooms">
										<label for="bedrooms"><?php _e('Bed Rooms',NOO_TEXT_DOMAIN); ?></label>
								...
								<div class="col-md-4">
									<div class="form-group s-prop-bathrooms">
										<label for="bathrooms"><?php _e('Bath Rooms',NOO_TEXT_DOMAIN); ?></label>
						...
						<div class="noo-control-group">
							<div class="group-title">
								<?php _e('Property Images', NOO_TEXT_DOMAIN); ?>
							</div>
							<div class="group-container row">
								<div class="col-md-12">
									<div id="upload-container">
										<div id="aaiu-upload-container">
											<?php noo_upload_form( $gallery, $featured_img, true ); ?>
											<input type="hidden" name="gallery" id="gallery" value="<?php echo $featured_img.','.$gallery;?>">
											<input type="hidden" name="featured_img" id="featured_img" value="<?php echo $featured_img;?>">
											<a id="aaiu-uploader" class="btn btn-secondary btn-lg" href="#"><?php _e('Select Images',NOO_TEXT_DOMAIN);?></a>
											<p><?php _e('At least 1 image is required for a valid submission. The featured image will be used to dispaly on property listing page.',NOO_TEXT_DOMAIN);?></p>
										</div>
									</div>
								</div>
							</div>
						...
						<div class="noo-control-group">
							<div class="group-title">
								<?php _e('Additional Info', NOO_TEXT_DOMAIN); ?>
							</div>
							<div class="group-container row">
						...
						<div class="noo-control-group">
							<div class="group-title">
								<?php _e('Listing Location', NOO_TEXT_DOMAIN); ?>
							</div>
							<div class="group-container row">
								<div class="col-md-8">
									<div class="form-group s-prop-address">
										<label for="address"><?php _e('Address',NOO_TEXT_DOMAIN); ?>&nbsp;*</label>
								...
								<div class="col-md-4">
									<div class="form-group s-prop-location">
										...
										<div class="dropdown">
					   						...
					   						?>
					   						<span class="location-label" data-toggle="dropdown"><?php _e('None',NOO_TEXT_DOMAIN)?></span>
					   						<?php
					   						endif;
					   						?>
							   				<?php 
					   						noo_dropdown_search(array(
						   						...
						   						'show_option_none'=>__('None',NOO_TEXT_DOMAIN),
												'hide_empty'=>0,
						   						'show_count'=>0,
							   				));?>
									...
									<div class="form-group s-prop-sub_location">
										<label><?php _e('Sub Location',NOO_TEXT_DOMAIN); ?></label>
										<div class="dropdown">
							   				<?php 
					   						...
					   						?>
					   						<span class="sub_location-label" data-toggle="dropdown"><?php _e('None',NOO_TEXT_DOMAIN)?></span>
					   						<?php
					   						endif;
					   						?>
								...
								<div class="col-md-6">
									<div class="form-group s-prop-lat">
										<label for="_noo_property_gmap_latitude"><?php _e('Latitude (Google Maps)',NOO_TEXT_DOMAIN); ?></label>
								...
								<div class="col-md-6">
									<div class="form-group s-prop-long">
										<label for="_noo_property_gmap_longitude"><?php _e('Longitude (Google Maps)',NOO_TEXT_DOMAIN); ?></label>
						...
						<div class="row">
							<div class="col-md-5">
								...
								<div class="noo-control-group small-group">
									<div class="group-title">
										<?php _e('Featured Submission', NOO_TEXT_DOMAIN); ?>
									</div>
									<div class="group-container row">
										<div class="col-md-12">
											<div class="form-group s-prop-featured">
												<?php if( $featured ) : ?>
												<span class="label label-success"><?php _e('Featured Property', NOO_TEXT_DOMAIN); ?></span>
								...
								<div class="noo-control-group small-group">
									<div class="group-title">
										<?php _e('Property Video', NOO_TEXT_DOMAIN); ?>
									</div>
									<div class="group-container row">
										<div class="col-md-12">
											<div class="form-group s-prop-video">
												<label for="_video_embedded"><?php _e('Video Embedded',NOO_TEXT_DOMAIN); ?></label>
												<input type="text" id="_video_embedded" class="form-control" value="<?php echo $video; ?>" name="video" />
												<small><?php echo sprintf( __('Enter a Youtube, Vimeo, Soundcloud, etc... URL. See supported services at %s', NOO_TEXT_DOMAIN), '<a href="http://codex.wordpress.org/Embeds" target="_BLANK">http://codex.wordpress.org/Embeds</a>'); ?>
												</small>
											</div>
										</div>
									</div>
							...
							<div class="col-md-7">
								<div class="noo-control-group small-group">
									<div class="group-title">
										<?php _e('Amenities & Features', NOO_TEXT_DOMAIN); ?>
									</div>
									<div class="group-container row">
						...
						<div class="noo-submit row">
							<div class="col-md-12">
								...
								<label><?php _e('Your submission will be reviewed by Administrator before it can be published', NOO_TEXT_DOMAIN); ?></label>
								<?php elseif( $need_approve && $action == 'edit') : ?>
								<label><?php _e('Your property will be unpublished for Administrator to review your changes', NOO_TEXT_DOMAIN); ?></label>
								<?php endif; ?>
							</div>
						</div>  
