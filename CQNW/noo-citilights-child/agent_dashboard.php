			...
			<div class="<?php noo_main_class(); ?>" role="main">
				<div class="properties list my-properties">
					<div class="properties-header">
						<h1 class="page-title"><?php _e('My Properties', NOO_TEXT_DOMAIN); ?></h1>
					</div>
					...
					<?php if ( $wp_query->have_posts() ) : ?>
						<?php while ( $wp_query->have_posts() ) : $wp_query->the_post(); ?>
							...
							$edit_link		= add_query_arg( 'prop_edit', $post_ID, $submit_link );
							...
							if( $membership_type == 'submission' ) {
								...
								$zero_price_text	= NooProperty::format_price( 0, 'text');
								...
								$total_price_text	= NooProperty::format_price( $total_price, 'text' );
							}
							?>
							...
							<div class="property-wrap">
								...
								<div class="property-labels">
									<?php if( $membership_type == 'submission' ) : ?>
										<?php if( !$paid_listing ) : ?>
									<span class="label label-danger"><?php _e('Not Paid', NOO_TEXT_DOMAIN); ?></span>
										<?php else : ?>
									<span class="label label-info"><?php _e('Paid', NOO_TEXT_DOMAIN); ?></span>
										<?php endif; ?>
									<?php endif; ?>
									<?php //if( $featured ) : ?>
									<!-- <span class="label label-success"><?php _e('Featured', NOO_TEXT_DOMAIN); ?></span> -->
									<?php //endif; ?>
									<?php if( $post_status == 'pending' ) : ?>
									<span class="label label-warning"><?php _e('Waiting for Approval', NOO_TEXT_DOMAIN); ?></span>
									<?php endif; ?>
								</div>
									...
									<div class="submission-payment" data-total-price-text="<?php echo $total_price_text; ?>" data-zero-price-text="<?php echo $zero_price_text; ?>" >
										<div class="payment-form">
											...
											<p class="listing-price">
												<label><?php _e('Submission Fee', NOO_TEXT_DOMAIN); ?>:<?php echo $membership_info['data']['listing_price_text'] ?></label>
											</p>
											<?php endif; ?>
											<?php if( !$featured ) : ?>
											<p class="form-group featured-price">
												<label for="submission_featured_<?php echo $post_ID; ?>">
													<input type="checkbox" class="submission-featured" name="submission_featured" id="submission_featured_<?php echo $post_ID; ?>"/>&nbsp;<?php _e('Featured Fee', NOO_TEXT_DOMAIN); ?>:<?php echo $membership_info['data']['featured_price_text']; ?>
													<i></i>
												</label>
											</p>
											<?php endif; ?>
										...
										<div class="payment-total">
											<strong><?php echo ( !$paid_listing ) ? $membership_info['data']['listing_price_text'] : NooProperty::format_price( 0 ); ?></strong>
										</div>
										<div class="payment-action">
											<input type="submit" class="paypal-btn btn btn-default <?php echo ( !$paid_listing ) ? '' : 'disabled'; ?>" value="<?php _e('Pay with Paypal', NOO_TEXT_DOMAIN);?>" />
										</div>
									...
									<div class="property-info">
										...
										<div class="property-action">
											<div data-prop-id="<?php echo $post_ID; ?>" data-agent-id="<?php echo $agent_id; ?>" class="agent-action <?php echo ( ( $membership_type == 'membership' ) ? 'four-buttons' : '' ); ?>">
												...
												<a title="<?php _e('Edit this Property',NOO_TEXT_DOMAIN);?>" href="<?php echo $edit_link;?>"><i class="fa fa-pencil"></i></a>
													<?php if( $is_sold ) : ?>
												<a class="active" title="<?php _e('Sold/Rent',NOO_TEXT_DOMAIN);?>" href="javascript:void(0)"><i class="fa fa-check"></i></a>
													<?php else : ?>
												<a class="status-property" title="<?php _e('Mark As Sold/Rent',NOO_TEXT_DOMAIN);?>" href="javascript:void(0)"><i class="fa fa-check"></i></a>
													<?php endif; ?>
												<?php else : ?>
												<a class="disabled" title="<?php _e('Can\'t edit this property',NOO_TEXT_DOMAIN);?>" href="javascript:void(0)"><i class="fa fa-pencil"></i></a>
												<a class="disabled" title="<?php _e('Can\'t edit this property',NOO_TEXT_DOMAIN);?>" href="javascript:void(0)"><i class="fa fa-check"></i></a>
												<?php endif; ?>
												<?php if( $membership_type == 'membership' ) : ?>
												<?php if( $featured ) : ?>
													<a class="active" title="<?php _e('Featured Property',NOO_TEXT_DOMAIN);?>" href="javascript:void(0)"><i class="fa fa-star"></i></a>
												<?php elseif( !$can_set_featured ) : ?>
													<a class="disabled" title="<?php _e('Can\'t set featured',NOO_TEXT_DOMAIN);?>" href="javascript:void(0)"><i class="fa fa-star"></i></a>
												<?php else : ?>
													<a class="featured-property" title="<?php _e('Make this a featured Property.',NOO_TEXT_DOMAIN);?>" href="javascript:void(0)"><i class="fa fa-star"></i></a>
												<?php endif; ?>
												<?php endif; ?>
												<a class="delete-property" title="<?php _e('Delete this Property',NOO_TEXT_DOMAIN);?>" href="javascript:void(0)"><i class="fa fa-times"></i></a>
											</div>
										</div>
									</div>
								</div>
						...
						<?php endwhile; ?>
					<?php else : ?>
						<h4><?php _e('You don\'t have any properties yet!', NOO_TEXT_DOMAIN); ?></h4>
					<?php endif;
					wp_reset_query();
					wp_reset_postdata();
					$noo_show_sold = false;
