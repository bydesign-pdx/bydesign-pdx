...
if(!class_exists('NooAgent')) :
	class NooAgent {
						...
						<div class="agent-wrap">
							<div class="agent-summary">
								<div class="agent-info">
									...
									<?php endif; ?>
									<?php if( !empty( $mobile ) ) : ?>
										<div><i class="fa fa-tablet"></i>&nbsp;<?php echo $mobile; ?></div>
									<?php endif; ?>
									<?php if( !empty( $email ) ) : ?>
										<div><i class="fa fa-envelope-square"></i>&nbsp;<?php echo $email; ?></div>
									<?php endif; ?>
									<?php if( !empty( $skype ) ) : ?>
										<div><i class="fa fa-skype"></i>&nbsp;<?php echo $skype; ?></div>
									<?php endif; ?>
								</div>
		...
		public function login_register_shortcode($atts, $content = null){
			...
			$class = ( $class != '' ) ? ' class="' . esc_attr( $class ) . '"' : '';
			$custom_style = ( $custom_style != '' ) ? ' style="' . $custom_style . '"' : '';
			$col_class = ( $mode == 'both' ) ? 'col-md-6' : 'col-md-12';
	...
	class NooMembership {
			...
			$meta_box = array(
				...
				'fields'       => array(
					array(
						...
						'std'     => '0',
						'options' => array(
							array('value'=>'0','label'=>__('Not Paid')),
							array('value'=>'1','label'=>__('Paid'))
						)
					),
				),
			);
		...
		public static function render_metabox_fields( $post, $id, $type, $meta, $std = null, $field = null ) {
			switch( $type ) {
				...
				case 'addable_text':
					$max_fields = 5;
					if ( !empty( $field['max_fields'] ) && is_numeric( $field['child-fields'] ) ) {
		...
		public function pre_get_posts( $query ) {
			...
			if( isset($query->query_vars['post_type']) && $query->query_vars['post_type'] == 'noo_agent' ) {
				...
				if( self::get_membership_type() == 'none' ) {
					return;
				}
