...
function noo_add_customizer_menu() {
	...
	add_menu_page( __( 'Customizer', NOO_TEXT_DOMAIN ), __( 'Customizer', NOO_TEXT_DOMAIN ), 'edit_theme_options', 'customize.php', null, $customizer_icon, 61 );
...
?>
    <style type="text/css"><?php echo noo_get_option( 'noo_custom_css', '' ); ?></style>
  <?php
	endif;
}

add_action( 'wp_head', 'noo_theme_option_custom_css', 999, 0 );
...
if ( ! function_exists( 'noo_customizer_controls_l10n' ) ) :
	function noo_customizer_controls_l10n() {
		return array(
			'navbar_height' => __( 'NavBar Height (px)', NOO_TEXT_DOMAIN),
			'mobile_navbar_height' => __( 'Mobile NavBar Height (px)', NOO_TEXT_DOMAIN),
			'ajax_update_msg'    => __( 'Updating ...', NOO_TEXT_DOMAIN ),
			'import_error_msg' => __( 'Error when parsing your file.', NOO_TEXT_DOMAIN ),
			'export_preparing_msg' => __( 'We are preparing your export file, please wait...', NOO_TEXT_DOMAIN ),
			'export_fail_msg' => __( 'There was a problem generating your export file, please try again.', NOO_TEXT_DOMAIN ),
...
if ( ! function_exists( 'noo_customizer_live_data' ) ) :
	function noo_customizer_live_data() {
		...
		return array(
			...
			'ok'					=> __( 'Yes', NOO_TEXT_DOMAIN ),
			'cancel'				=> __( 'No', NOO_TEXT_DOMAIN ),
			'ajax_update_msg'		=> __( 'Updating ...', NOO_TEXT_DOMAIN ),
			'cannot_preview_msg'	=> __( 'This option doesn\'t support live preview. Save it and see the change on your site.', NOO_TEXT_DOMAIN ),
			'redirect_msg'			=> __( 'Wanna go to %s to see the change?', NOO_TEXT_DOMAIN ),
			'blog_page_txt'			=> __( 'Blog Page', NOO_TEXT_DOMAIN ),
			// 'shop_page_txt'			=> __( 'Shop Page', NOO_TEXT_DOMAIN ),
			'archive_page_txt'		=> __( 'An Archive Page', NOO_TEXT_DOMAIN ),
			'post_page_txt'			=> __( 'A Post', NOO_TEXT_DOMAIN ),
			// 'product_page_txt'		=> __( 'A Product', NOO_TEXT_DOMAIN ),
			'agents_page_txt'		=> __( 'Agents Archive Page', NOO_TEXT_DOMAIN ),
			'properties_page_txt'	=> __( 'Properties Archive Page', NOO_TEXT_DOMAIN ),
			'property_page_txt'		=> __( 'Property Detail Page', NOO_TEXT_DOMAIN ),
		);
	}
endif;
...
if ( ! function_exists( 'noo_jbst_tmpadminheader' ) ) :
	function noo_jbst_tmpadminheader() {
		...
		$title = __('Customizer credentials', NOO_TEXT_DOMAIN);
