...
function noo_render_metabox_fields ( $post, $id, $type, $meta, $std, $field = null ) {
	switch( $type ) {
		...
		case 'gallery':
			...
			$btn_text = !empty( $meta ) ? __( 'Edit Gallery', NOO_TEXT_DOMAIN ) : __( 'Add Images', NOO_TEXT_DOMAIN );
			...
			echo '<input type="button" class="button" name="' . $id . '_button_clear" id="' . $id . '_clear" value="' . __( 'Clear Gallery', NOO_TEXT_DOMAIN ) . '" />';
...
?>
				...
				jQuery(document).ready(function($) {
					...
					$('#<?php echo $id; ?>_upload').on('click', function(event) {
						...
						wp_media_frame.on('update', function(){
							...
							noo_upload_btn.attr('value', '<?php echo __( 'Edit Gallery', NOO_TEXT_DOMAIN ); ?>');
					...
					$('#<?php echo $id; ?>_clear').on('click', function(event) {
						...
						noo_clear_btn.hide();
						$('#<?php echo $id; ?>_upload').attr('value', '<?php echo __( 'Add Images', NOO_TEXT_DOMAIN ); ?>');
		...
		case 'image':
			...
			echo '<input type="button" class="button button-primary" name="' . $id . '_button_upload" id="' . $id . '_upload" value="' . $btn_text . '" />';
			echo '<input type="button" class="button" name="' . $id . '_button_clear" id="' . $id . '_clear" value="' . __( 'Clear Image', NOO_TEXT_DOMAIN ) . '" />';
			echo '<div class="noo-thumb-wrapper">' . $output . '</div>';
?>
			<script>
				jQuery(document).ready(function($) {
					...
					$('#<?php echo $id; ?>_clear').on('click', function(event) {
						var noo_clear_btn = $(this);
						noo_clear_btn.hide();
						$('#<?php echo $id; ?>_upload').attr('value', '<?php echo __( 'Select Image', NOO_TEXT_DOMAIN ); ?>');
						noo_clear_btn.siblings('#<?php echo $id; ?>').val('');
						noo_clear_btn.siblings('.noo-thumb-wrapper').html('');
					});
				});
...
?>
		...
		case 'page_layout':
			...
			switch( $post_layout ) {
				case 'fullwidth':
					$post_layout_text = __( 'Full-Width Page', NOO_TEXT_DOMAIN );
					break;
				case 'sidebar':
					$post_layout_text = __( 'Page With Right Sidebar', NOO_TEXT_DOMAIN );
					break;
				case 'left_sidebar':
					$post_layout_text = __( 'Page With Left Sidebar', NOO_TEXT_DOMAIN );
					break;
			}
			
			echo '<p>' . sprintf( __( 'Global setting for the Layout of Single Post page is: <strong>%s</strong>', NOO_TEXT_DOMAIN), $post_layout_text ) . '</p>';
			if ( $post_layout != 'fullwidth' ) {
				$sidebar_text = get_sidebar_name( $sidebar );
				echo '<p>' . sprintf( __( 'And the Sidebar is: <strong>%s</strong>', NOO_TEXT_DOMAIN), $sidebar_text ) . '</p>';
			}
		...
		case 'menus':
			...
			echo'<select name="noo_meta_boxes[' . $id . ']" >';
			echo'	<option value="" '. selected( $meta, '', true ) . '>' . __('Don\'t Need Menu', NOO_TEXT_DOMAIN) . '</option>';
		...
		case 'users':
			...
			echo'<select name="noo_meta_boxes[' . $id . ']" >';
			echo'	<option value="" '. selected( $meta, '', true ) . '>' . __('No User', NOO_TEXT_DOMAIN) . '</option>';
		...
		case 'rev_slider':
			...
			echo '<select name="noo_meta_boxes[' . $id . ']">';
			echo '<option value="">' . __(' - No Slider - ', NOO_TEXT_DOMAIN) . '</option>';
