...
// 				$heading = noo_get_option( 'noo_blog_heading_title', __( 'My Blog', NOO_TEXT_DOMAIN ) );
...
// 				$heading = noo_get_option( 'noo_portfolio_heading_title', __( 'My Portfolio', NOO_TEXT_DOMAIN ) );
...
// 				$heading = noo_get_option( 'noo_shop_heading_title', __( 'My Shop', NOO_TEXT_DOMAIN ) );
// 				$sub_heading = noo_get_option( 'noo_shop_heading_sub_title', '' );
// 			}
// 		} elseif ( is_search() ) {
// 			$heading = __( 'Search Results', NOO_TEXT_DOMAIN );
// 			global $wp_query;
// 			if(!empty($wp_query->found_posts)) {
// 				if($wp_query->found_posts > 1) {
// 					$heading =  $wp_query->found_posts ." ". __('Search Results for:',NOO_TEXT_DOMAIN)." ".esc_attr( get_search_query() );
// 				} else {
// 					$heading =  $wp_query->found_posts ." ". __('Search Results for:',NOO_TEXT_DOMAIN)." ".esc_attr( get_search_query() );
// 				}
// 			} else {
// 				if(!empty($_GET['s'])) {
// 					$heading = __('Search Results for:',NOO_TEXT_DOMAIN)." ".esc_attr( get_search_query() );
// 				} else {
// 					$heading = __('To search the site please enter a valid term',NOO_TEXT_DOMAIN);
// 				}
// 			}
// 		} elseif ( is_author() ) {
// 			$curauth = (get_query_var('author_name')) ? get_user_by('slug', get_query_var('author_name')) : get_userdata(get_query_var('author'));
// 			$heading = __('Author Archive',NOO_TEXT_DOMAIN)." ";

// 			if(isset($curauth->nickname)) $heading .= __('for:',NOO_TEXT_DOMAIN)." ".$curauth->nickname;
...
// 			}
// 		} elseif ( is_year() ) {
//     		$heading = __( 'Post Archive by Year: ', NOO_TEXT_DOMAIN ) . get_the_date( 'Y' );
// 		} elseif ( is_month() ) {
//     		$heading = __( 'Post Archive by Month: ', NOO_TEXT_DOMAIN ) . get_the_date( 'F,Y' );
// 		} elseif ( is_day() ) {
//     		$heading = __( 'Post Archive by Day: ', NOO_TEXT_DOMAIN ) . get_the_date( 'F j, Y' );
// 		} elseif ( is_404() ) {
//     		$heading = __( 'Oops! We could not find anything to show to you.', NOO_TEXT_DOMAIN );
//     		$sub_heading =  __( 'Would you like going else where to find your stuff.', NOO_TEXT_DOMAIN );
// 		}
