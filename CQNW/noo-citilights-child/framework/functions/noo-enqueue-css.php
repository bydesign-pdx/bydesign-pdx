		...
		if ( ! is_admin() ) {
			...
			if( $typo_use_custom_font ) {
				...
				if ( ! empty( $body_font_family ) && $body_font_family != $default_font ) {
					...
					$body_font_style         = noo_get_option( 'noo_typo_body_font_style', $default_font_weight );
				...
				if ( ! empty( $headings_font_family ) && $headings_font_family != $default_font ) {
					$headings_font_weight    = noo_get_option( 'noo_typo_headings_font_weight', '400' ) == '' ? '400' : noo_get_option( 'noo_typo_headings_font_weight', '400' );
					$headings_font_style     = noo_get_option( 'noo_typo_headings_font_style', $default_font_weight );
			...
			if( $nav_custom_font ) {
				$nav_font_family      = noo_get_option( 'noo_typo_nav_font', $default_font );

				if ( ! empty( $nav_font_family ) && $nav_font_family != $default_font ) {
					$nav_font_weight  = noo_get_option( 'noo_typo_nav_font_weight', $default_font_weight ) == '' ? $default_font_weight : noo_get_option( 'noo_typo_nav_font_weight', $default_font_weight );
					$nav_font_style   = noo_get_option( 'noo_typo_nav_font_style', $default_font_weight );
					$nav_font_subset  = noo_get_option( 'noo_typo_nav_font_subset', $default_font_subset ) == '' ? 'latin' : noo_get_option( 'noo_typo_nav_font_subset', $default_font_subset );
			...
			if( ! $use_image_logo ) {
				...
				if ( !empty( $logo_font_family ) && $logo_font_family != $default_font ) {
					$logo_font_weight     = noo_get_option( 'noo_header_logo_font_weight', $default_font_weight ) == '' ? $default_font_weight : noo_get_option( 'noo_header_logo_font_weight', $default_font_weight );
					$logo_font_style      = noo_get_option( 'noo_header_logo_font_style', '700' );
