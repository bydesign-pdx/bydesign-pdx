			...
			?>
				...
				<div class="searchHidden" id="ajax_search_container">
					<form class="form-inline" id="ajax_searchform" action="<?php echo home_url();?>">
						<input autocomplete="off" type="text" class="form-control" id="s" name="s" value="<?php if (isset($_REQUEST['s'])) echo $_REQUEST['s']; ?>">
						<span id="ajax-search-loading"></span>
					</form>
					<div class="ajax_results"></div>
				</div>

				<?php
	        $form = ob_get_clean();
	        */
	        $items .= '<li id="nav-menu-item-search" class="menu-item noo-menu-item-search"><a class="search-button" href="#"><i class="fa fa-search"></i></a><div class="searchbar hide">'.get_search_form(false).'</div></li>';
	    }
	    return $items;
	}
