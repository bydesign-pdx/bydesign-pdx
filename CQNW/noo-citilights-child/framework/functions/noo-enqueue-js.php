...
if ( ! function_exists( 'noo_enqueue_site_scripts' ) ) :
	function noo_enqueue_site_scripts() {
		...
		wp_register_script( 'vendor-carouFredSel-touchSwipe', NOO_FRAMEWORK_URI . '/vendor/carouFredSel/helper-plugins/jquery.touchSwipe.min.js', array( 'jquery' ), null, true );
		wp_register_script( 'vendor-carouFredSel', NOO_FRAMEWORK_URI . '/vendor/carouFredSel/jquery.carouFredSel-6.2.1-packed.js', array( 'jquery', 'vendor-carouFredSel-touchSwipe' ), null, true );
		...
		wp_register_script( 'noo-property-map', NOO_ASSETS_URI . '/js/property-map.js', array( 'jquery','vendor-form' , 'google-map' , 'google-map-infobox','google-map-markerclusterer','jquery-ui-slider'), null, true );
		...
		wp_localize_script('noo-img-uploader', 'noo_img_upload', array(
			...
			'max_files' => 0,
			'upload_enabled' => true,
			'confirmMsg' => __('Are you sure you want to delete this?', NOO_TEXT_DOMAIN),
			'plupload' => array(
				...
				'filters' => array(array('title' => __('Allowed Files', NOO_TEXT_DOMAIN), 'extensions' => "jpg,gif,png")),
				'multipart' => true,
		...
		wp_localize_script( 'noo-dashboard', 'noo_dashboard', array(
			...
			'confirmDeleteMsg' => __('Are you sure you want to delete this Property? This action can\'t be undone.', NOO_TEXT_DOMAIN),
			'confirmFeaturedMsg' => __('The number of featured items will be subtracted from your package. This action can\'t be undone. Are you sure you want to do it?', NOO_TEXT_DOMAIN),
			'confirmStatusMsg' => __('Are you sure you want to mark this property as Sold/Rent? This action can\'t be undone.', NOO_TEXT_DOMAIN),
			));
		...
		if ( ! is_admin() ) {
			...
			if( class_exists( 'NooProperty' ) ) {
				$latitude = NooProperty::get_google_map_option('latitude','40.714398');
				$longitude = NooProperty::get_google_map_option('longitude','-74.005279');
				$nooGmapL10n = array(
					'ajax_url'        => admin_url( 'admin-ajax.php', 'relative' ),
					...
					'zoom'=>NooProperty::get_google_map_option('zoom',12),
					'fitbounds'=>NooProperty::get_google_map_option('fitbounds','1') ? true : false,
					'area_unit' => NooProperty::get_general_option('area_unit'),
					'thousands_sep' => wp_specialchars_decode( stripslashes(NooProperty::get_general_option('price_thousand_sep')),ENT_QUOTES),
					'decimal_sep' => wp_specialchars_decode( stripslashes(NooProperty::get_general_option('price_decimal_sep')),ENT_QUOTES),
					'num_decimals' => NooProperty::get_general_option('price_num_decimals'),
					'currency'=>NooProperty::get_currency_symbol(NooProperty::get_general_option('currency')),
					'currency_position'=>NooProperty::get_general_option('currency_position','left'),
					'default_label'=>'',
					'fullscreen_label'=>'',
					...
					'markers'=>NooProperty::get_properties_markers(),
					'ajax_finishedMsg'=>__('All posts displayed',NOO_TEXT_DOMAIN),
				);
				wp_localize_script('noo-property', 'nooGmapL10n', $nooGmapL10n);

				// if( $is_agents || $is_property || $is_properties ) {
