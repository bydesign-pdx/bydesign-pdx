	...
	function noo_get_featured_image($prefix = '_noo_wp_post', $post_id = null, $lightbox=false, $is_shortcode = false) {
		...
		if(!empty($thumb)) {
			if (!is_singular() || $lightbox) {
				$html[] = '<a class="content-thumb'.( $lightbox ? ' noo-lightbox-item':'').'" '.($lightbox ? 'data-lightbox-gallery="portfolio-gallery"':'').'  href="' .($lightbox ? $post_thumbnail_src : esc_url(get_permalink())) . '" title="' . esc_attr(sprintf(__('Permalink to: "%s"', NOO_TEXT_DOMAIN) , the_title_attribute('echo=0'))) . '">';
				$html[] = $thumb;
				$html[] = '</a>';
			} else {
		...
		if(!empty($gallery_ids)) {			
			...
			foreach ($gallery_arr as $index => $image_id) {
				...
				if(!empty($thumb)) {
					...
					if($lightbox)
						$html[] = '<a class="'.( $lightbox ? ' noo-lightbox-item':'').'" '.($lightbox ? 'data-lightbox-gallery="portfolio-gallery-'.$post_id.'"':'').'  href="' .($lightbox ? $post_thumbnail_src : '') . '" title="' . esc_attr(sprintf(__('Permalink to: "%s"', NOO_TEXT_DOMAIN) , the_title_attribute('echo=0'))) . '">';
					$html[] = $thumb;
					if($lightbox)
						$html[] = '</a>';
	...
	function noo_get_featured_video($prefix = '_noo_wp_post', $post_id = null,$lightbox=false,  $is_shortcode = false) {
		...
		if (!is_singular() || $is_shortcode) {
			if ($preview_content == 'featured' && has_post_thumbnail( $post_id )) {
				...
				if(!empty($thumb)) {
					$html[] = '<a class="content-thumb '.( $lightbox ? ' noo-lightbox-item':'').'" '.($lightbox ? 'data-lightbox-type="inline"':'').'  href="' .($lightbox ? '#noo-video-container'.$post_id : esc_url(get_permalink())) . '" title="' . esc_attr(sprintf(__('Permalink to: "%s"', NOO_TEXT_DOMAIN) , the_title_attribute('echo=0'))) . '">';
					$html[] = $thumb;
					$html[] = '</a>';
				}
		...
		if($lightbox)
			$html[] = '<a class="hide'.( $lightbox ? ' noo-lightbox-item':'').'" '.($lightbox ? 'data-lightbox-type="inline"':'').'  href="#noo-video-container'.$post_id.'" title="' . esc_attr(sprintf(__('Permalink to: "%s"', NOO_TEXT_DOMAIN) , the_title_attribute('echo=0'))) . '"></a>';
					...
					<div class="jp-controls-container jp-video">
						<div id="jp_interface_<?php echo $post_id; ?>" class="jp-interface">
							<ul class="jp-controls">
								<li><a href="#" class="jp-play" tabindex="1"><span><?php echo __('Play',NOO_TEXT_DOMAIN) ?></span></a></li>
								<li><a href="#" class="jp-pause" tabindex="1"><span><?php echo __('Pause',NOO_TEXT_DOMAIN) ?></span></a></li>
								<li><a href="#" class="jp-mute" tabindex="1"><span><?php echo __('Mute',NOO_TEXT_DOMAIN) ?></span></a></li>
								<li><a href="#" class="jp-unmute" tabindex="1"><span><?php echo __('UnMute',NOO_TEXT_DOMAIN) ?></span></a></li>
							</ul>
	...
	function noo_get_featured_default($post_id = null) {
		...
		if (has_post_thumbnail()) {
			...
			if (is_singular()) {
				...
				$html[] = '<a class="content-thumb" href="' . esc_url(get_permalink()) . '" title="' . esc_attr(sprintf(__('Permalink to: "%s"', NOO_TEXT_DOMAIN) , the_title_attribute('echo=0'))) . '">';
				$html[] = $thumb;
				$html[] = '</a>';
			}
		}
