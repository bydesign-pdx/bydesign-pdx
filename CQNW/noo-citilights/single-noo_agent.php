				...
				<article id="post-<?php the_ID(); ?>" class="noo-agent">
					...
					<div class="agent-info">
						...
						<div class="agent-detail">
							...
							<div class="agent-detail-info">
								<?php if( !empty( $phone ) ) : ?>
									<div class="agent-phone"><i class="fa fa-phone"></i>&nbsp;<span><?php _e('Phone:',NOO_TEXT_DOMAIN)?></span><?php echo $phone; ?></div>
								<?php endif; ?>
								<?php if( !empty( $mobile ) ) : ?>
									<div class="agent-mobile"><i class="fa fa-tablet"></i>&nbsp;<span><?php _e('Mobile:',NOO_TEXT_DOMAIN)?></span><?php echo $mobile; ?></div>
								<?php endif; ?>
								<?php if( !empty( $email ) ) : ?>
									<div class="agent-email"><i class="fa fa-envelope-square"></i>&nbsp;<span><?php _e('Email:',NOO_TEXT_DOMAIN)?></span><?php echo $email; ?></div>
								<?php endif; ?>
								<?php if( !empty( $skype ) ) : ?>
									<div class="agent-skype"><i class="fa fa-skype"></i>&nbsp;<span><?php _e('Skype:',NOO_TEXT_DOMAIN)?></span><?php echo $skype; ?></div>
								<?php endif; ?>
							</div>
					...
					<div class="conact-agent row">
						...
						<form role="form" id="conactagentform" method="post">
							...
							<?php 
							$fields = array(
								'name'=>'<div class="form-group col-md-6 col-sm-6"><input type="text" name="name" class="form-control" placeholder="'.__('Your Name *',NOO_TEXT_DOMAIN).'"></div>',
								'email'=>'<div class="form-group  col-md-6 col-sm-6"><input type="email" name="email" class="form-control" placeholder="'.__('Your Email *',NOO_TEXT_DOMAIN).'"></div>',
								'message'=>'<div class="form-group message col-md-12 col-sm-12"><textarea name="message" class="form-control" rows="5" placeholder="'.__('Message *',NOO_TEXT_DOMAIN).'"></textarea></div>',
							);
