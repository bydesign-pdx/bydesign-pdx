...
if( !class_exists('nooPayPalFramework')) :
	class nooPayPalFramework
	{
		...
		private function __construct() {
			...
			$this->_url = $this->getSetting('enable_sandbox', false) ? 'https://www.sandbox.paypal.com/webscr' : 'https://www.paypal.com/webscr';
			$this->_notify_url = add_query_arg( array( 'action' => 'paypal_listener' ), admin_url('admin-ajax.php') );
		...
		public function hashCall( $args ) {
			...
			if ( !is_wp_error($resp) && $resp['response']['code'] >= 200 && $resp['response']['code'] < 300 ) {
				...
				$message = __( 'Request:', NOO_TEXT_DOMAIN );
				$message .= "\r\n".print_r($request, true)."\r\n\r\n";
				$message .= __( 'Response:', NOO_TEXT_DOMAIN );
				$message .= "\r\n".print_r(wp_parse_args( $resp['body'] ), true)."\r\n\r\n";
				$this->_notify_mail( __( 'PayPal Framework - hashCall sent successfully', NOO_TEXT_DOMAIN ), $message );
				...
				$message .= __( 'Response:', NOO_TEXT_DOMAIN );
				$message .= "\r\n".print_r($resp, true)."\r\n\r\n";
				$this->_notify_mail( __( 'PayPal Framework - hashCall failed', NOO_TEXT_DOMAIN ), $message );
				if ( !is_wp_error($resp) )
		...
		private function _validateMessage() {
			...
			unset( $_POST['cmd'] );
			$message = __('URL:', NOO_TEXT_DOMAIN );
			$message .= "\r\n".print_r($this->_url, true)."\r\n\r\n";
			$message .= __('Options:', NOO_TEXT_DOMAIN );
			$message .= "\r\n".print_r($this->_settings, true)."\r\n\r\n";
			$message .= __('Response:', NOO_TEXT_DOMAIN );
			$message .= "\r\n".print_r($resp, true)."\r\n\r\n";
			$message .= __('Post:', NOO_TEXT_DOMAIN );
			$message .= "\r\n".print_r($_POST, true);
			...
			if ( !is_wp_error($resp) && $resp['response']['code'] >= 200 && $resp['response']['code'] < 300 && (strcmp( $resp['body'], "VERIFIED") == 0)) {
				// $this->_notify_mail( __( 'IPN Listener Test - Validation Succeeded', NOO_TEXT_DOMAIN ), $message );
				return true;
			} else {
				// If we can't validate the message, assume it's bad
				// $this->_notify_mail( __( 'IPN Listener Test - Validation Failed', NOO_TEXT_DOMAIN ), $message );
				return false;
			}
		}
