...
if (!function_exists('noo_content_meta')):
	function noo_content_meta() {
		...
		// 			$categories_html .= '<a' . ' href="' . get_term_link($category->slug, 'portfolio_category') . '"' . ' title="' . esc_attr(sprintf(__("View all Portfolio Items in: &ldquo;%s&rdquo;", NOO_TEXT_DOMAIN) , $category->name)) . '">' . '<i class="nooicon-bookmark"></i> ' . $category->name . '</a>' . $separator;
		// 		}
		// 	}
		// } else {
		$categories = get_the_category();
		foreach ($categories as $category) {
			$categories_html.= '<a' . ' href="' . get_category_link($category->term_id) . '"' . ' title="' . esc_attr(sprintf(__("View all posts in: &ldquo;%s&rdquo;", NOO_TEXT_DOMAIN) , $category->name)) . '">' . '<i class="nooicon-archive"></i> ' . $category->name . '</a>' . $separator;
		}
		// }
...
?>
		<li id="li-comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
			<div class="comment-wrap">
				<div class="comment-img">
					...
					<div class="ispostauthor">
						<?php _e('Post<br/>Author', NOO_TEXT_DOMAIN); ?>
					</div>
					<?php
		endif; ?>
				</div>
			...
			'reply_text' => (__('Reply', NOO_TEXT_DOMAIN) . ' <span class="comment-reply-link-after"><i class="nooicon-reply"></i></span>') ,
			'depth' => $depth,
			'max_depth' => $args['max_depth']
						...
						<div class="comment-meta">
							...
							<span class="comment-edit">
								<?php edit_comment_link('<i class="nooicon-edit"></i> ' . __('Edit', NOO_TEXT_DOMAIN)); ?>
							</span>
						</div>
