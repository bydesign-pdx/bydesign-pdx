...
function noo_customizer_add_custom_controls( $wp_customize ) {
	...
	class NOO_Customizer_Control_Checkbox extends WP_Customize_Control {
		...
		public function __construct( $manager, $id, $args = array(), $options = array() ) {
			...
			if ( isset( $args['data_on'] ) && !empty( $args['data_on'] ) ) {
				...
				$this->data_on = __( 'ON', NOO_TEXT_DOMAIN );
			}
			
			if ( isset( $args['data_off'] ) && !empty( $args['data_off'] ) ) {
				$this->data_off = $args['data_off'];
			} else {
				$this->data_off = __( 'OFF', NOO_TEXT_DOMAIN );
			}
		}
		...
		?>
		<label>
			<input id="noo-checkbox-<?php echo $this->id; ?>" type="checkbox" value="<?php echo esc_attr( $this->value() ); ?>" <?php $this->link(); checked( $this->value() ); ?> />
		...
		function _get_inline_script() {
			...
			jQuery(window).load(function() {
				...
				?>
				jQuery('#customize-control-<?php echo trim( $on_option ); ?>').addClass('child_<?php echo $this->id; ?> <?php echo $this->id; ?>_val_on');
				<?php
					endforeach;
				endif;
				?>
				...
				?>
				jQuery('#customize-control-<?php echo trim( $off_option ); ?>').addClass('child_<?php echo $this->id; ?> <?php echo $this->id; ?>_val_off');
				<?php
					endforeach;
				endif;
				?>
				var customize_control    = jQuery('#customize-control-<?php echo $this->id; ?>');
				...
				customize_control.bind("toggle_children", function() {
					...
					if($this.css('display') == 'none') {
						jQuery('.child_<?php echo $this->id; ?>').css("display", "none").trigger("toggle_children");
					...
					if(checkbox.is( ':checked' )) {
						jQuery('.<?php echo $this->id; ?>_val_off').css("display", "none").trigger("toggle_children");
						jQuery('.<?php echo $this->id; ?>_val_on').css( 'display', 'block' ).trigger("toggle_children");
					} else {
						jQuery('.<?php echo $this->id; ?>_val_on').css("display", "none").trigger("toggle_children");
						jQuery('.<?php echo $this->id; ?>_val_off').css( 'display', 'block' ).trigger("toggle_children");
					}
				});
	...
	class NOO_Customizer_Control_Switch extends NOO_Customizer_Control_Checkbox {
		...
		<div class="noo-switch">
			<input id="noo-switch-<?php echo $this->id; ?>" type="checkbox" value="<?php echo esc_attr( $this->value() ); ?>" <?php $this->link(); checked( $this->value() ); ?> />
			<label for="noo-switch-<?php echo $this->id; ?>" data-on="<?php echo $this->data_on; ?>" data-off="<?php echo $this->data_off; ?>"></label>
		</div>
		<?php // Add script if there's child-options setting.
			$this->_get_inline_script();
		}
	...
	class NOO_Customizer_Control_Radio extends WP_Customize_Control {
        ...
        <script>
          jQuery(window).load(function() {
			...
			?>
                jQuery('#customize-control-<?php echo trim( $child_option ); ?>').addClass('child_<?php echo $this->id; ?> <?php echo $this->id; ?>_val_<?php echo $option_value; ?>');
                <?php
			endforeach;
			endforeach;
		?>
            ...
            customize_control.bind("toggle_children", function() {
            	...
            	jQuery('.child_<?php echo $this->id; ?>:not(.<?php echo $this->id; ?>_val_' + checkedElement.val() + ')')
            		.css("display", "none")
            		.trigger("toggle_children");
            	jQuery('.<?php echo $this->id; ?>_val_' + checkedElement.val())
            		.css( 'display', 'block' )
            		.trigger("toggle_children");
            });
	...
	class NOO_Customizer_Control_Same_As_Radio extends WP_Customize_Control {
        ...
        <?php
			...
			foreach ( $this->choices as $value => $label ) :
		?>
          <label <?php if($same_as_value == $this->value()) echo ' style="display:none;"';?>>
          ...
          <input type="hidden" name="<?php echo esc_attr( $name ); ?>" value="<?php echo $this->value(); ?>" <?php $this->link(); ?>>
          <?php
			endforeach;
		...
		?>
            ...
            customize_control.find('input[type=checkbox]').click( function() {
            	...
            	if($checkbox.is( ':checked' )) {
            		...
            		customize_control.find('input[type=radio]').each(function() {
            			$radio = jQuery(this);
            			$radio.attr('name', '<?php echo $name; ?>')
            				.attr('data-customize-setting-link', '<?php echo $this->id; ?>');
	...
	class NOO_Customize_Alpha_Color extends WP_Customize_Control {
	    ...
	    public function render_content() { ?>
	        <label>
	            <span class="customize-control-title"><?php echo esc_html( $this->label ); ?></span>
	            <input type="text" data-palette="<?php echo $this->palette; ?>" data-default-color="<?php echo $this->default; ?>" value="<?php echo intval( $this->value() ); ?>" class="noo-color-control" <?php $this->link(); ?>  />
	        </label>
	    <?php }
	}
	...
	class NOO_Customize_Image_Control extends WP_Customize_Image_Control {
		...
		public function __construct( $manager, $id, $args = array(), $options = array() ) {
			...
			$this->add_tab( 'library',  __( 'Media Library', NOO_TEXT_DOMAIN ), array( $this, 'media_library' ) );
		...
		public function media_library() {
			?>
				<a class="choose-from-library-link button" data-controller="<?php echo $this->id; ?>">
					<?php _e( 'Open Library', NOO_TEXT_DOMAIN ); ?>
				</a>
			<?php
		}
	...
	class NOO_Customizer_Control_Textarea extends WP_Customize_Control {
		...
		public function render_content() {
			...
			<label>
				<span class="customize-control-title"><?php echo esc_html( $this->label ); ?></span>
				<textarea <?php $this->link(); ?> rows="<?php echo $row; ?>" style="width: 98%;"><?php echo $this->value(); ?></textarea>
			</label>
			<?php
		}
	}
	...
	class NOO_Customizer_Control_Widgets_Select extends WP_Customize_Control {
      ...
      <label>
        ...
        <select <?php $this->link(); ?>>
          <option value=""><?php echo __( '- Select Sidebar -', NOO_TEXT_DOMAIN ); ?></option>
          <?php
	...
	class NOO_Customizer_Control_Pages_Select extends WP_Customize_Control {
		...
		public function render_content() {
			...
			$dropdown = str_replace( '<select', '<select ' . $this->get_link() . 'class="noo-customize-chosen" data-placeholder="' . __( '- Select Page -', NOO_TEXT_DOMAIN ) . '"', $dropdown );
		?>
      <label>
        <span class="customize-control-title"><?php echo esc_html( $this->label ); ?></span>
        <?php echo $dropdown; ?>
	...
	class NOO_Customizer_Control_Posts_Select extends WP_Customize_Control {
		...
		<label>
				...
				<option value="" <?php selected('', $this->value()); ?>><?php echo $select_holder; ?></option>
	...
	class NOO_Customizer_Control_Google_Fonts extends WP_Customize_Control {
		...
		public function __construct( $manager, $id, $args = array(), $options = array() ) {
			...
			if ( empty ( self::$font_weight_name ) )
				self::$font_weight_name = array(
					'100'       => __( 'Extra Light', NOO_TEXT_DOMAIN ),
					'100italic' => __( 'Extra Light Italic', NOO_TEXT_DOMAIN ),
					'200'       => __( 'Light', NOO_TEXT_DOMAIN ),
					'200italic' => __( 'Light Italic', NOO_TEXT_DOMAIN ),
					'300'       => __( 'Book', NOO_TEXT_DOMAIN ),
					'300italic' => __( 'Book Italic', NOO_TEXT_DOMAIN ),
					'400'       => __( 'Regular', NOO_TEXT_DOMAIN ),
					'400italic' => __( 'Regular Italic', NOO_TEXT_DOMAIN ),
					'500'       => __( 'Medium', NOO_TEXT_DOMAIN ),
					'500italic' => __( 'Medium Italic', NOO_TEXT_DOMAIN ),
					'600'       => __( 'Semi-bold', NOO_TEXT_DOMAIN ),
					'600italic' => __( 'Semi-bold Italic', NOO_TEXT_DOMAIN ),
					'700'       => __( 'Bold', NOO_TEXT_DOMAIN ),
					'700italic' => __( 'Bold Italic', NOO_TEXT_DOMAIN ),
					'800'       => __( 'Extra-bold', NOO_TEXT_DOMAIN ),
					'800italic' => __( 'Extra-bold Italic', NOO_TEXT_DOMAIN ),
					'900'       => __( 'Heavy', NOO_TEXT_DOMAIN ),
					'900italic' => __( 'Heavy Italic', NOO_TEXT_DOMAIN ),
				);
		}
		...
		?>
          ...
          <div class="noo-font-selects">
            <select class="noo-font-family noo-customize-chosen" data-placeholder="<?php _e( '- Font Family -', NOO_TEXT_DOMAIN ); ?>" <?php $this->link(); ?>>
            <option value=""></option>
              <?php
				$selected_font_id = 0;
		...
		?>
            ...
            <select class="noo-font-weight-and-style">
              <option value=""><?php echo __( '- Styles -', NOO_TEXT_DOMAIN ); ?></option>
              <?php
		...
		?>
            ...
            <select class="noo-font-subset" data-customize-setting-link="<?php echo $this->id . '_subset'; ?>">
              <option value=""><?php echo __( '- Subsets -', NOO_TEXT_DOMAIN ); ?></option>
              <?php
		...
		?>
            ...
            <input type="hidden" class="noo-font-weight" value="<?php echo $this->weight; ?>" data-customize-setting-link="<?php echo $this->id . '_weight'; ?>">
            <input type="hidden" class="noo-font-style" value="<?php echo $this->style; ?>" data-customize-setting-link="<?php echo $this->id . '_style'; ?>">
          </div>
        <?php
			}
		}
