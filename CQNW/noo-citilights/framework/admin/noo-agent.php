		...
		public function save_post( $post_id, $post ) {
			...
			if( self::has_associated_user( $post_id ) ) {
				...
				if ( $sanitized_user_login == '' ) {
					$has_error = true;
					$err_message[] = ( __( 'Please enter a username.', NOO_TEXT_DOMAIN ) );
				} elseif ( ! validate_username( $user_login ) ) {
					$has_error = true;
					$err_message[] = (  __( 'This username is invalid because it uses illegal characters. Please enter a valid username.', NOO_TEXT_DOMAIN ) );
				} elseif ( username_exists( $sanitized_user_login ) ) {
					$has_error = true;
					$err_message[] = ( __( 'This username is already registered. Please choose another one.', NOO_TEXT_DOMAIN ) );
				}

				if ( empty($email) ) {
					$has_error = true;
					$err_message[] = ( __( 'Please type your e-mail address.', NOO_TEXT_DOMAIN ) );
				} elseif ( !is_email( $email ) ) {
					$has_error = true;
					$err_message[] = ( __( 'The email address isn\'t correct.', NOO_TEXT_DOMAIN ) );
				} elseif ( email_exists( $email ) ) {
					$has_error = true;
					$err_message[] = ( __( 'This email is already registered, please choose another one.', NOO_TEXT_DOMAIN ) );
				}
				...
				if( !$has_error ) {
					...
					if( is_wp_error( $user_id ) || empty( $user_id ) ) {
						$has_error = true;
						$err_message[] = ( __('There\'s an unknown error. Please retry or contact Administrator.', NOO_TEXT_DOMAIN) );
					} else {
