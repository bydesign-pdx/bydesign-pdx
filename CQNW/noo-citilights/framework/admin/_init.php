...
if (!function_exists('noo_init_theme')):
	function noo_init_theme() {
	    ...
	    $locale = apply_filters('theme_locale', get_locale(), NOO_TEXT_DOMAIN);

	    load_textdomain(NOO_TEXT_DOMAIN, WP_LANG_DIR.'/noo-citilights/'.NOO_TEXT_DOMAIN.'-'.$locale.'.mo');

		load_theme_textdomain( NOO_TEXT_DOMAIN, get_template_directory() . '/languages' );
		...
		if (noo_get_option('noo_top_bar', 'html') == 'menu') {
			$menu_list['top-menu'] = __('Top Menu', NOO_TEXT_DOMAIN);
		}
		
		$menu_list['primary'] = __('Primary Menu', NOO_TEXT_DOMAIN);
		
		if (noo_get_option('noo_bottom_bar_menu', false)) {
			$menu_list['footer-menu'] = __('Footer Menu', NOO_TEXT_DOMAIN);
		}
