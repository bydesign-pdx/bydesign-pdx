	...
	class NooProperty{
		...
		public function advanced_search_property_shortcode($atts, $content = null){
			...
			<div class="noo_advanced_search_property <?php echo $style.' '.$class?>" <?php echo $custom_style?>>
				<?php self::advanced_map( !$disable_map, '', true, '', false, $no_search_container, $source,!!$disable_search_form,$show_advanced_search_field, $map_height)?>
			</div>
			<?php
			return ob_get_clean();
		}
								...
								<article <?php post_class(); ?>>
									<div class="property-featured">
										...
										<div class="property-detail">
											<div class="size list"><span><?php echo self::get_area_html(get_the_ID());?></span></div>
                                    		<div class="size grid"><span><?php echo noo_get_post_meta(null,'_address')?></span></div>
...
// 			$custom_field_tmpl.= '</td>';
			    ...
			    <div class="property-featured clearfix">
			    	...
			    	<div class="thumbnails">
			    		<div class="thumbnails-wrap">
				    		<ul>
				    		<li>
				    			<a data-rel="0" href="<?php echo $image[0]?>"><?php echo get_the_post_thumbnail(get_the_ID(), 'property-thumbnail') ?></a>
				    		</li>
				    		<?php $i = 1;?>
				    		<?php foreach ($gallery_ids as $gallery_id): $gallery_image = wp_get_attachment_image_src($gallery_id,'property-full')?>
				    		<li>
				    			<a data-rel="<?php echo $i ?>" href="<?php echo $gallery_image[0]?>"><?php echo wp_get_attachment_image( $gallery_id, 'property-thumbnail'); ?></a>
				    		</li>
				    		<?php $i++;?>
				    		<?php endforeach;?>
				    		</ul>
						...
						<div class="property-detail col-md-4 col-sm-4">
							...
							<div class="property-detail-content">
								...
								<?php foreach ((array)$custom_fields  as $field):?> 
                                    ...
                                    <?php //CUSTOM - check if pdf link ?>
									<?php if(!empty($custom_field_value) && strpos($field['name'],'lyer') === false):?>
                                    ...
                                    <?php // Custom PDF link ?>
										...
										elseif(!empty($custom_field_value) && strpos($field['name'],'lyer') !== false):
									...
									<span class="col-sm-7 detail-field-value <?php echo sanitize_title(@$field['name'])?>"><?php echo "<a href='" . $custom_field_value . "' target='_blank'>Download</a>"; ?></span>
		 ...
		 */
		public static function display_content($query='',$title='',$display_mode  = true,$default_mode = '',$show_pagination = false,$ajax_pagination=false,$show_orderby=false,$ajax_content=false){
			self::enqueue_gmap_js();
			wp_enqueue_script('noo-property');
