...
function formStylization() {
	...
	$('input[type="radio"]').not('.stars .star').wrap('<div class="new-radio"></div>');
...
function optionHomeMap() {
	var $                  = jQuery,
		...
		filterButtonWidth  = filterButton.outerWidth(),
	...
	if (( windowHeight > ( headerHeight + filterHeight + tabHeight + 60 )) && $('.home-filter').not('.filter-position-right')) {
	...
	if ($(filter).hasClass('open')) {
		...
		$(filterButton).html(flatroom.showfilter + '<div class="flip"><span>+</span></div>');
		$(filter).css({
	...
	filterButton.click(function(){
		...
		if ($(filter).hasClass('open')) {
			$('#main .home-filter .filter-content').fadeOut('400',
			function() {
				filterButton.html(flatroom.showfilter + '<div class="flip"><span>+</span></div>');
				filterButtonWidth  = filterButton.outerWidth();
				filter.animate({
			...
			$(this).html(flatroom.hide + ' <div class="flip"><span>&#8211;</span></div>');
			filterButtonWidth  = filterButton.outerWidth();
			if ($(filter).hasClass('filter-position-right')) {
				...
				function(){
					$('#main .home-filter .filter-content').fadeIn('400');
				});
			} else {
				var dataTop    = $(this).parent().data('posTop');
				var dataLeft   = $(this).parent().data('posLeft');
				var filterTop  = (typeof dataTop != 'undefined') ? dataTop : marginTop;
				var filterLeft = (typeof dataLeft != 'undefined') ? dataLeft : ((windowWidth / 2) + (filterWidth / 2) - filterButtonWidth);
...
//Idx Carousel
function carouselIdx() {
	var $ = jQuery;
	$('.dsidx-widget-listings .dsidx-slideshow ul').each(function () {
		$(this).carouFredSel({
			auto: false,
			infinite  : false,
			next: $(this).closest('.dsidx-slideshow').find('.dsidx-move-down'),
			prev: $(this).closest('.dsidx-slideshow').find('.dsidx-move-up'),
			swipe: {
				onMouse: true,
				onTouch: true,
				easing: 'linear'
...
function language() {
	var $               = jQuery,
		bodyWidth       = $('body').width(),
		logoWidth       = $('.logo-box').outerWidth(),
		iconWidth       = $('.header-icons').outerWidth(),
		loginWidth      = $('.login-register').outerWidth(),
		languageWidth   = $('.language-box').outerWidth(),
		menuButtonWidth = 0,
		box             = $('.header-right-box');

	if (!$('body').hasClass('main-menu-visible')) {
		menuButtonWidth = $('.menu-button-box').outerWidth();
		
		if (logoWidth + iconWidth + loginWidth + languageWidth + menuButtonWidth > bodyWidth) {
			box.addClass('two-column');
		} else {
			box.removeClass('two-column');
		}
	}
	
	if ($('body').width() >= 768) {
		box.css('maxWidth', bodyWidth - logoWidth - iconWidth - loginWidth);
		$('.login-register').css('left', 'inherit');
	} else {
		box.css('maxWidth', 'inherit');
		$('.login-register').css('left', languageWidth);
	}
}
...
function videoRespons() {
	...
	if ($('.embedded iframe').length) {
		...
		video
			.css({
				height : (videoHeight * videoBox.width()) / videoWidth,
...
jQuery(document).ready(function() {
	...
	/* Carousels */
	$('.dsidx-widget-listings .dsidx-controls').click(function(){
		carouselIdx();
	});
	$(window).bind({
		load : function() {
	...
	//IDX
	$('#dsidx-contact-form-submit').addClass('btn');
	$('.dsidx-results #dsidx-map-control a').wrap('<div class="map-title">Map</div>');
	$('#dsidx-secondary-data, .dsidx-supplemental-data').wrap('<div class="table-over">Map</div>');
	...
	//Calendar Widget
	$('#wp-calendar #today').wrapInner('<div></div>');
	...
	$('.team-shortcode .team-member .worker-info').bind('touchend', function(e){
