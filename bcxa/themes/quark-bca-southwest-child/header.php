	...
	<div class="visuallyhidden skip-link"><a href="#primary" title="<?php esc_attr_e( 'Skip to main content', 'quark' ); ?>"><?php esc_html_e( 'Skip to main content', 'quark' ); ?></a></div>
		...
		$frontpage_id = get_option('page_on_front'); 
	 	if (has_post_thumbnail( $post->ID ) ): 
			//This page has featured image
			$image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'single-post-thumbnail' );
			$image = $image[0]; 
		elseif (has_post_thumbnail( $frontpage_id ) ) :
			//Home page has featured image
			$image = wp_get_attachment_image_src( get_post_thumbnail_id( $frontpage_id ), 'single-post-thumbnail' );
			$image = $image[0];
		else :
			//defualt
			$image = get_bloginfo( 'stylesheet_directory') . '/images/header-background-1.jpg'; 
		endif; 
		?>
	<div id="headercontainer" style="background-image: url('<?php echo $image; ?>')">
